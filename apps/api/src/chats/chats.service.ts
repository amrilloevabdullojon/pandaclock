...
export class ChatsService {
  ...
  async ensureMembership(channelId: string, userId: string): Promise<void> {
    ...
    const rows = await client.$queryRawUnsafe<{ channel_id: string }[]>(
      `SELECT channel_id FROM chat_members WHERE channel_id = $1::uuid AND user_id = $2::uuid LIMIT 1`,
      channelId,
      userId,
    );
    if (rows.length === 0) {
  ...
  async sendMessage(
    ...
    await client.$executeRawUnsafe(
      `UPDATE chat_members SET last_read_at = NOW() WHERE channel_id = $1::uuid AND user_id = $2::uuid`,
      channelId,
      authorId,
    );
